...
const salvarContato = () => {
  ...
  contato = { id: 0, nome: nome.val(), email: email.val() };

  if(id.val() !== "") {
    var i = listaContatos.findIndex(x => x.id == id.val());
    contato.id = id.val();
    listaContatos.splice(i,1,constato);
  } else {
    let idInsert = tamLista + 1;
    contato.id = idInsert;
    listaContatos.push(constato);
  }
...
const removeItemList = (id) => {
    var i = listaContatos.findIndex(x => x.id === id);
